...
export function StatisticsAreaChart({ data }: StatisticsAreaChartProps) {
  ...
  useEffect(() => {
    ...
    const maxValue = Math.max(...data.map((d) => Math.max(d.value, d.trend)), 3000000)

    // Draw horizontal grid lines
    ctx.strokeStyle = "#e5e7eb"
    ctx.lineWidth = 1

    const gridLines = 3 // 0, 1M, 2M, 3M
    ...
    data.forEach((point, index) => {
      ...
      // Add small circle for trend line points
      if (index > 0) {
        ctx.fillStyle = "#ec4899"
        ctx.beginPath()
        ctx.arc(x + barWidth / 2, chartHeight - (point.trend / maxValue) * chartHeight, 4, 0, Math.PI * 2)
        ctx.fill()
